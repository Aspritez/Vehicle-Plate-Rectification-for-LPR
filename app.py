"""Thai license-plate deskew app — runs locally or on Streamlit Community Cloud."""
...
def slider_with_info(label: str, minimum: Any, maximum: Any, step: Any, key: str, description: str) -> None:
    """Render an adjustment control with a tooltip."""
...
with st.sidebar:
    if IS_STREAMLIT_CLOUD:
        st.success("☁️ Streamlit Community Cloud", icon="☁️")
    else:
        st.info("🖥️ Running locally — [http://localhost:8501](http://localhost:8501)", icon="🖥️")
...
settings_col, action_col = st.columns([1, 1.2], gap="large")
...
with action_col:
    ...
    if detect_clicked:
        ...
        with st.expander("คะแนนบริเวณที่อาจเป็นป้ายทะเบียน", expanded=False):
