...
**Streamlit app (อัปเดตล่าสุด):** ฝังหน้าเว็บต้นฉบับ `frontend/` ทั้งหน้าเป็น custom component (`components.declare_component(path=frontend)`) โหมดที่หน้าเว็บคุยกับ Python ผ่านโปรโตคอล component ของ Streamlit (`StreamlitBridgeClient` ใน `api_client.js`, ตรวจโหมดจาก path `/component/`) แทน HTTP ส่วน Python (`streamlit_app.py`) เป็นตัวเชื่อมบางๆ เรียก `PlateReader` GUI จึงเหมือนเวอร์ชัน FastAPI ทุกอย่าง (ลากกรอบ, ลากมุม 4 จุด, Snap, แท็บ pipeline) และไฟล์ front-end ชุดเดียวใช้ทั้งสองเวอร์ชัน · เพิ่ม: พื้นที่ crop มืดลงสดตอนลากค้าง + ปุ่มดาวน์โหลดทุกขั้น/zip (ตัวเขียน zip ใน `js/zip.js` ไม่ใช้ไลบรารี) · เวอร์ชันก่อนหน้าที่ใช้ `streamlit-image-coordinates` ถูกเลิกใช้ (ส่งค่าตอนปล่อยเมาส์เท่านั้น โชว์สดไม่ได้) · `streamlit-drawable-canvas` ตัวดั้งเดิมพังกับ Streamlit รุ่นใหม่ จึงไม่ใช้ · ทดสอบแล้วในเบราว์เซอร์: Car433 → `กด 3789` เพชรบุรี, Car435 ลากกรอบ → `ภย 4644` (อัตโนมัติอ่านผิดเป็น `บ 8`), zip ที่ดาวน์โหลดผ่านการตรวจ CRC ด้วย Python
